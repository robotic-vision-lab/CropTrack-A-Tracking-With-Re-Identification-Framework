...
class GrapeDataset():
    ...
    def __init__(self, data_base_dir, split):
        ...
        test_sequences = [ 'CloseUp1', 'CloseUp2', 'Overview1', 'Overview2']
        train_sequences = [ 'vid23_05','vid23_04', 'vid25_04', 'vid26_05', 'vid26_04']
        ...
        self.sequence = []
        for s in sequences:
            self.sequence.append(GrapeSequence(s, data_base_dir))
...
class GrapeSequence():
    ...
    def __init__(self, seq_name, data_base_dir):
        ...
        #assert seq_name in os.listdir(self.seq_dir), 'Image set does not exist: {}'.format(seq_name)
    ...
    def __getitem__(self, idx):
        ...
        data = self.data[idx]
        img = None#io.imread(data['im_path']) #Image.open(data['im_path']).convert("RGB")
        ...
        sample['gt'] = data['gt']
        sample['vis'] = data['vis']
        sample['feats'] = np.array(data['feats'])
    ...
    def _sequence(self):
        ...
        img_dir = osp.join(self.seq_dir, 'Images')
        gt_path = osp.join(self.seq_dir, 'gt', 'gt.txt')
        det_path = osp.join(self.seq_dir, 'dets', 'det.txt')
        feat_path = osp.join(self.seq_dir, 'dets', 'extracted_det_bbox_features.npy')
        # det_path = osp.join(self.seq_dir, 'gt', 'gt.txt')
        # feat_path  = osp.join(self.seq_dir, 'gt', 'extracted_det_bbox_features.npy')
        #det_path = osp.join(self.seq_dir, seq_name+'.txt')
        ...
        if osp.exists(feat_path):
            bbox_feat = np.load(feat_path)
            features_exist = True

        if osp.exists(det_path):
            with open(det_path, "r") as inf:
                reader = csv.reader(inf, delimiter=',')
                for row, f in zip(reader, bbox_feat):
